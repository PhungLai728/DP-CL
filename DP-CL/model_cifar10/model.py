...
#from dpsgd.dp_mnist import Train
...
class Model:
    ...
    def __init__(self, x_train, y_, num_tasks, opt, imp_method, synap_stgth, fisher_update_after, fisher_ema_decay, network_arch='FC-S', 
        ...
        # print('self.class_attr',self.class_attr)
        # exit()
...
####################################################################################
    def loss_and_train_ops_for_one_hot_vector(self, x, y_):
        ...
        if self.network_arch == 'FC-S':
            ...
            if self.imp_method == 'PNN':
                ...
                self.unweighted_entropy = []
                for i in range(self.num_tasks):
                    self.task_pruned_logits.append(tf.where(tf.tile(tf.equal(self.output_mask[None,:], 1.0), [tf.shape(logits)[0], 1]), logits, NEG_INF*tf.ones_like(logits)))
                    ...
                    adjusted_entropy = tf.reduce_sum(tf.cast(tf.tile(tf.equal(self.output_mask[None,:], 1.0), [tf.shape(y_)[0], 1]), dtype=tf.float32) * y_, axis=1) * cross_entropy
    ...
    # def resnet18_conv_feedforward(self, x, kernels, filters, strides):
    #     """
    #     Forward pass through a ResNet-18 network
    ...
    #     """
    #     self.trainable_vars = []
    #     self.x_image = tf.reshape(x, [-1,28,28,3]);
    #     # autoencoder
    #     # self.Enc_Layer = EncLayer(self.x_image, kernels[0], filters[0], strides[0], self.trainable_vars, name='autoenc_1', collect=AECODER_VARIABLES)
        
    #     # print('strides', strides)
    #     # exit()
    #     with tf.variable_scope('conv1') as scope:
    #         # h = _conv(h, kernels[0], filters[0], strides[0], self.trainable_vars, name='conv_1')
    #         # h = tf.nn.conv2d(self.x_image, self.trainable_vars[0], [1, strides[0], strides[0], 1], padding="SAME") 
    #         h = _conv(self.x_image, kernels[0], filters[0], strides[0], self.trainable_vars, name='conv_1', collect=AECODER_VARIABLES)
    #         h = _bn(h, self.trainable_vars, self.train_phase, name='bn_1', collect=CONV_VARIABLES)
    #         cv1 = tf.nn.relu(h)
    #     # print(cv1)
    #     # Conv2_x
    #     h = _residual_block(cv1, self.trainable_vars, self.train_phase, name='conv2_1', collect=CONV_VARIABLES)
    #     current1 = tf.concat((cv1, h), axis=3)
    #     print(current1)
    #     h = _residual_block(current1, self.trainable_vars, self.train_phase, name='conv2_2', collect=CONV_VARIABLES)
        
    #     # Conv3_x
    #     h = _residual_block_first(h, filters[2], strides[2], self.trainable_vars, self.train_phase, name='conv3_1', is_ATT_DATASET=self.is_ATT_DATASET, collect=CONV_VARIABLES)
    #     current2 = tf.concat((cv1, h), axis=3)
    #     print(current2)
    #     h = _residual_block(current2, self.trainable_vars, self.train_phase, name='conv3_2', collect=CONV_VARIABLES)
    #     print(h)

    #     # Conv4_x
    #     h = _residual_block_first(h, filters[3], strides[3], self.trainable_vars, self.train_phase, name='conv4_1', is_ATT_DATASET=self.is_ATT_DATASET, collect=CONV_VARIABLES)
    #     print(h)
    #     current3 = tf.concat((tf.reshape(cv1, [-1, 7, 7, 256]), h), axis=3)
    #     h = _residual_block(current3, self.trainable_vars, self.train_phase, name='conv4_2', collect=CONV_VARIABLES)

    #     # Conv5_x
    #     h = _residual_block_first(h, filters[4], strides[4], self.trainable_vars, self.train_phase, name='conv5_1', is_ATT_DATASET=self.is_ATT_DATASET, collect=CONV_VARIABLES)
    #     h = _residual_block(h, self.trainable_vars, self.train_phase, name='conv5_2', collect=CONV_VARIABLES)
        
    #     #FC1
    #     print(h)
    #     h_flat = tf.reshape(h, [-1, 4*4*160])
    #     h_fc = _fc(h_flat, 256, self.trainable_vars, name = 'fc1', is_cifar = True, collect=CONV_VARIABLES)
    #     # Apply average pooling
    #     #h = tf.reduce_mean(h, [1, 2])
    #     # h_clipped = tf.clip_by_value(h_fc, -1.0, 1.0)
    #     # print(h_clipped)
    #     # Store the feature mappings
    #     self.features = h_fc #h_clipped
    #     self.image_feature_dim = h_fc.get_shape().as_list()[-1]
    ...
    #     else:
    #         if self.network_arch == 'RESNET-S':
    #             self.hk = h_fc.get_shape().as_list()[1]
    #             logits = _fc(h_fc, self.total_classes, self.trainable_vars, name='fc_2', is_cifar=True, collect=CONV_VARIABLES)
    #         else:
    #             logits = _fc(h_fc, self.total_classes, self.trainable_vars, name='fc_2', collect=CONV_VARIABLES)
    #         return logits
    ...
    def create_stochastic_gem_ops(self):
        ...
        sensitivity = 1
        gradient_l2norm_bound = sensitivity
        sigma = 1.7
        batch_size = 100
        
        #sensitivity = 0.01
        #gradient_l2norm_bound = 4.0
        #sigma = 1
        
        #print("IN THE model.py FILE, GOT SENSITIVITY")
        ref_grads = tf.gradients(self.agem_loss, self.trainable_vars)
        # print("ref_grads")
        # print(ref_grads)
        # print("ref_grads shape")
        # print(len(ref_grads))
        # exit()
        ...
        for i in range(len(ref_grads)):
            ...
            grad1 += tf.random_normal(shape=tf.shape(grad1), mean=0.0,
        ...
        #print("STORING REF GRADIENTS IN MODEL.py FILE")
        self.store_ref_grads = [tf.assign(ref, g) for ref, g in zip(self.ref_grads, clip_grads_ref)]
        # self.testing_reference_gradient = ref
        # self.testing_clipped_gradient = g
        ...
        # print(len(self.first_task_gradients_vars))
        # exit()

        all_w = []
        all_grad = []
        for i in range(len(self.first_task_gradients_vars)):
        ...
        # print(list(zip(all_grad, all_w)))
        # exit()
        self.train_first_task = self.opt.apply_gradients(zip(all_grad, all_w))



        # computed_gradient1 = self.first_task_gradients_vars[0][0]
        # weight1 = self.first_task_gradients_vars[0][1]
        
        # computed_gradient2 = self.first_task_gradients_vars[1][0]
        # weight2 = self.first_task_gradients_vars[1][1]
        
        # computed_gradient3 = self.first_task_gradients_vars[2][0]
        # weight3 = self.first_task_gradients_vars[2][1]
        
        # computed_gradient4 = self.first_task_gradients_vars[3][0]
        # weight4 = self.first_task_gradients_vars[3][1]
        
        # computed_gradient5 = self.first_task_gradients_vars[4][0]
        # weight5 = self.first_task_gradients_vars[4][1]
