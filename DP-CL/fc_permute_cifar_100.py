...
from model_cifar100 import Model
...
def test_task_sequence(model, sess, test_data, test_tasks, task, classes_per_task=0):
    ...
    for tt, labels in enumerate(test_tasks):
        ...
        if model.imp_method == 'PNN':
            ...
            logit_mask[:] = 0
            logit_mask[tt][labels] = 1.0
            # print('test_data image', len(task_test_images))
            # print('test_data labels', len(task_test_labels))
            # print('test_data image', len(task_test_images[0]))
            # print('test_data labels', len(task_test_labels[0]))
            # exit()
            feed_dict = {model.x: task_test_images, 
                    model.y_: task_test_labels, model.keep_prob: 1.0, model.train_phase: False}
            logit_mask_dict = {m_t: i_t for (m_t, i_t) in zip(model.output_mask, logit_mask)}
            # print('logit_mask', type(logit_mask))
            # print('logit_mask', len(logit_mask))
            # print('logit_mask_dict', type(logit_mask_dict))
            # print('logit_mask_dict', len(logit_mask_dict))
            # exit()
            feed_dict.update(logit_mask_dict)
            acc = model.accuracy[tt].eval(feed_dict = feed_dict)
...
def main():
    ...
    time_end = time.time()
    time_spent = time_end - time_start
    print('all time for running', time_spent)
