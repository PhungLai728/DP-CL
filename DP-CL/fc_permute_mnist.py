...
def get_arguments():
    ...
    parser.add_argument("--eps-mem-batch", type=int, default=EPS_MEM_BATCH_SIZE,
                       help="Number of samples per class from previous tasks.")
    parser.add_argument("--examples-per-task", type=int, default=55000,
...
def train_task_sequence(model, sess, args):
    ...
    print("ENTERED TRAIN_TASK_SEQUENCE FROM MAIN")
    #print(model.imp_method)
    ...
    if model.imp_method == 'A-GEM' or model.imp_method == 'ER':
        ...
        #print("RECOGNIZED THAT EPISODIC MEMORY SHOULD BE TRUE")
    else:
        use_episodic_memory = False
        #print("DID NOT RECOGNISE THAT EPISODIC MEM SHOULD BE TRUE AS DID NOT RECOGNISE AGEM")
    ...
    for runid in range(args.num_runs):
        ...
        #print("model initialized with model.init_updates")
        ...
        if use_episodic_memory:
            ...
            #print("Space for episodic memory saved")
        ...
        for task in range(len(datasets)):
            ...
            if(task > 0 and model.imp_method != 'PNN'):
                model.restore(sess)
                #print("not the first task so restored weights from previous task using model.restore")
                ...
                if model.imp_method == 'VAN':
                    ...
                    if task == 0:
                        ...
                        if episodic_filled_counter <= args.eps_mem_batch:
                            ...
                            #print("random sample sampled from episodic memory buffer")
                        # Store the reference gradient
                        #print("need to store reference gradient")
                        randomTaskForGrefComputation = random.randrange(task)
                        print("RANDOM TASK" + str(randomTaskForGrefComputation))
                        #sess.run(model.store_ref_grads, feed_dict={model.x: episodicMemoryPerTask[randomTaskForGrefComputation][0][mem_sample_mask], model.y_: episodicMemoryPerTask[randomTaskForGrefComputation][1][mem_sample_mask],
                            #model.keep_prob: 1.0, model.output_mask: logit_mask, model.train_phase: True})
                        sess.run(model.store_ref_grads, feed_dict={model.x: episodicMemoryPerTask[randomTaskForGrefComputation][0], model.y_: episodicMemoryPerTask[randomTaskForGrefComputation][1],
                            model.keep_prob: 1.0, model.output_mask: logit_mask, model.train_phase: True})
                        #print("called model for gradient")
                        value_1, value_2 = sess.run([model.testing_reference_gradient, model.testing_clipped_gradient], feed_dict = feed_dict)
                        print("value 1")
                        print(value_1)
                        #value_2 = sess.run(model.testing_clipped_gradient, feed_dict = feed_dict)
                        print("value 2")
                        print(value_2)
                    ...
                    #print("episodicMemoryPerTask being added")
                    episodicMemoryPerTask[task] = [episodic_images,episodic_labels]
...
def main():
    ...
    with graph.as_default():
        ...
        #print("CREATING THE MODEL OBJECT model")
