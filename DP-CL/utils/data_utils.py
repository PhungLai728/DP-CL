...
def random_crop_and_pad_image(images, crop_h, crop_w):
    ...
    image_pad = tf.image.pad_to_bounding_box(images, 0, 0, tf.maximum(
        crop_h, image_shape[1]), tf.maximum(crop_w, image_shape[2]))
    img_crop = tf.map_fn(lambda img: tf.random_crop(
        img, [crop_h, crop_w, 3]), image_pad)
    return img_crop
...
def _AWA_get_data(data_dir, train_list_file, val_list_file, test_list_file, img_height, img_width):
    ...
    # you can change the number of validation images here TODO: Pass this as argument
    num_val_img = 0
    ...
    train_img, train_label = _AWA_read_img_from_file(
        data_dir, train_list_file, img_height, img_width)
    #validation_img, validation_label = _AWA_read_img_from_file(data_dir, val_list_file, img_height, img_width)
    test_img, test_label = _AWA_read_img_from_file(
        data_dir, test_list_file, img_height, img_width)
...
def construct_split_awa(task_labels, data_dir, train_list_file, val_list_file, test_list_file, img_height, img_width, attr_file=None):
    ...
    # Get the awa dataset
    awa_data = _AWA_get_data(data_dir, train_list_file,
                             val_list_file, test_list_file, img_height, img_width)
...
def _CUB_get_data(data_dir, train_list_file, test_list_file, img_height, img_width):
    ...
    train_img, train_label = _CUB_read_img_from_file(
        data_dir, train_list_file, img_height, img_width)
    test_img, test_label = _CUB_read_img_from_file(
        data_dir, test_list_file, img_height, img_width)
...
def construct_split_cub(task_labels, data_dir, train_list_file, test_list_file, img_height, img_width, attr_file=None):
    ...
    # Get the cub dataset
    cub_data = _CUB_get_data(data_dir, train_list_file,
...
def construct_permute_mnist(num_tasks):
    ...
    for i in range(num_tasks):
        ...
        for set_name in sets:
            ...
            this_set._images = np.transpose(
                np.array([this_set.images[:, c] for c in perm_inds]))
            if set_name == "train":
                train = {
...
def construct_permute_cifar100(data_dir, num_tasks, is_cifar_100=False):
    """
        Construct a dataset of permutted mnist images
    ...
    # Download the dataset if needed
    # _cifar_maybe_download_and_extract("Cifar")
    cifar10_data = cifar10_read.read_data_sets(data_dir, one_hot=True)

    permuttedCifar = []
    for i in range(num_tasks):
        # print(cifar10_data.train.images.shape)
        # print(cifar10_data.train.labels.shape)
        # permute one chanel
        numchanels = 0
        fix_inds = np.arange(int(numchanels*cifar10_data.train.images.shape[1]/3))
        perm_inds = np.arange(int(numchanels*cifar10_data.train.images.shape[1]/3), int(cifar10_data.train.images.shape[1]))
        perm_inds = list(perm_inds)
        fix_inds = list(fix_inds)
        np.random.shuffle(perm_inds)
        #print('no shuffle')
        fix_inds.extend(perm_inds)
        copied_cifar = deepcopy(cifar10_data)
        #sets = ['train', 'validation', 'test']
        sets = ['train', 'test']
        for set_name in sets:
            # this_set = getattr(copied_cifar, set_name) # shallow copy
            #this_set._images = np.transpose(np.array([this_set.images[:,c] for c in perm_inds]))
            if set_name == "train":
                this_set = copied_cifar.train
                this_set._images = np.transpose(
                    np.array([this_set.images[:, c] for c in fix_inds]))
                ...
                this_set = copied_cifar.validation
                this_set._images = np.transpose(
                    np.array([this_set.images[:, c] for c in fix_inds]))
                validation = {
                    'images': this_set._images,
        ...
        permuttedCifar.append(data)
    print('Lenght of Permutted Cifar: ' + str(len(permuttedCifar)))
    return permuttedCifar
