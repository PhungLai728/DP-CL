...
from utils.data_utils import construct_split_cifar, construct_permute_mnist, construct_permute_cifar100
...
## Training Options
NUM_RUNS = 3           # Number of experiments to average over
TRAIN_ITERS = 60      # Number of training iterations per task
BATCH_SIZE = 500
LEARNING_RATE = 0.5
RANDOM_SEED = 1234
...
#     W_conv1Noise = np.random.laplace(0.0, Delta2/(L*epsilon2), 3*image_size**2).astype(np.float32);
...
def get_arguments():
    ...
    parser.add_argument("--examples-per-task", type=int, default=30000,
...
def train_task_sequence(model, sess, datasets, args):
    ...
    for runid in range(args.num_runs):
        ...
        # shape     = model.trainable_vars[0].get_shape().as_list()
        # w_t       = tf.reshape(model.trainable_vars[0], [-1, shape[-1]])
        # w         = tf.transpose(w_t)
        # sing_vals = tf.svd(w, compute_uv=False)
        # sensitivity = tf.reduce_max(sing_vals)
        # gamma = 2*Delta2/(batch_size*sensitivity)
        # _gamma = sess.run(gamma)

        # epsilon2_update = epsilon2/(1.0 + 1.0/_gamma)
        # #print(epsilon2_update)

        # AdvLnoise = generateIdLMNoise(IMG_HEIGHT, Delta2, epsilon2_update, batch_size);
        # ##perturb h for training
        # perturbFM_h = np.random.laplace(0.0, 2*Delta2/(epsilon2_update*batch_size), 14*14*64)
        # perturbFM_h = np.reshape(perturbFM_h, [-1, 14, 14, 64]);
        ...
        for task in range(len(datasets)):
            ...
            """task_tr_images, task_tr_labels = load_task_specific_data(datasets[task]['train'], task_labels[task])
            task_val_images, task_val_labels = load_task_specific_data(datasets[task]['validation'], task_labels[task])
            ...
            print('Received {} images, {} labels at task {}'.format(datasets[task]['train']['images'].shape[0], datasets[task]['train']['labels'].shape[0], task))
            ...
            task_sample_weights = np.ones([datasets[task]['train']['labels'].shape[0]], dtype=np.float32)

            num_train_examples = datasets[task]['train']['images'].shape[0]
            # Randomly suffle the training examples
            """perm = np.arange(num_train_examples)
            np.random.shuffle(perm)
            train_x = task_train_images[perm][:task_train_labels.shape[0]]
            train_y = task_train_labels[perm][:task_train_labels.shape[0]]
            task_sample_weights = task_sample_weights[perm][:task_train_labels.shape[0]]"""
            ...
            perm = np.arange(num_train_examples)
            np.random.shuffle(perm)
            train_x = datasets[task]['train']['images']
            train_y = datasets[task]['train']['labels']
            #task_sample_weights = task_sample_weights[perm]

            # Array to store accuracies when training for task T
            ftask = []

            # Number of iterations after which is checked
            convergence_iters = int(num_iters * MEASURE_CONVERGENCE_AFTER)

            num_iters_actual = 518

            # Training loop for task T
            epoches = 5
            print('---- num_iters actual ----',num_iters_actual)
            # exit()
            for iters in range(epoches*num_iters):
                if iters <= num_iters_actual: 
                    # print('---- iters ----', iters)
                    if args.train_single_epoch and not args.cross_validate_mode and not MULTI_TASK:
                        if (iters <= 20) or (iters > 20 and iters % 50 == 0):
                            ...
                            fbatch = test_task_sequence(model, sess, datasets, task_labels)
                            ftask.append(fbatch)
                            """if model.imp_method == 'PNN':
                    ...
                    if args.train_single_epoch:
                        offset = (iters%num_iters) * batch_size
                        ...
                        if model.imp_method == 'PNN':
                            ...
                            feed_dict = {model.x: train_x[offset:offset+residual], model.y_: train_y[offset:offset+residual], 
                                    ...
                                    model.training_iters: num_iters, model.train_step: iters, model.keep_prob: 0.5, model.output_mask: logit_mask,
                    ...
                    if model.imp_method == 'VAN':
                        ...
                        if task == 0:
                            ...
                            # _, loss_AUTO = sess.run([model.train_first_task_AECODER, model.autoencoder_cost], feed_dict=feed_dict)
                            ...
                            if episodic_filled_counter <= args.eps_mem_batch*EPS_NUM_RANDOM_BATCH:
                                mem_sample_mask = np.arange(episodic_filled_counter)
                            else:
                                # Sample a random subset from episodic memory buffer
                                eps_bach_index = np.random.choice(int(episodic_filled_counter/args.eps_mem_batch), EPS_NUM_RANDOM_BATCH, replace=False)
                                mem_sample_mask = np.arange(args.eps_mem_batch) + eps_bach_index[0]*args.eps_mem_batch
                                for b_index in range(1, len(eps_bach_index)):
                                    mem_sample_mask = np.concatenate((mem_sample_mask, np.arange(args.eps_mem_batch) + eps_bach_index[b_index]*args.eps_mem_batch), axis=None)
                            ...
                            # sess.run(model.store_ref_grads_AECODER, feed_dict=ref_feed_dict)
                            ...
                            if COUNT_VIOLATONS:
                                ...
                                #vc2, _2, loss_AUTO = sess.run([model.violation_count_AECODER, model.train_subseq_tasks_AECODER, model.autoencoder_cost], feed_dict=feed_dict)
                            else:
                                _, loss = sess.run([model.train_subseq_tasks, model.agem_loss], feed_dict=feed_dict)
                                #_2, loss_AUTO = sess.run([model.train_subseq_tasks_AECODER, model.autoencoder_cost], feed_dict=feed_dict)
                    ...
                    if iters == (epoches*num_iters-1):
                        for er_x, er_y_ in zip(train_x[offset:offset+residual], train_y[offset:offset+residual]):
                            cls = np.unique(np.nonzero(er_y_))[-1]
                            # Write the example at the location pointed by count_cls[cls]
                            cls_to_index_map = cls #np.where(np.array(task_labels[task]) == cls)[0][0]
                    ...
                    if (iters % 50 == 0) or (iters % (num_iters_actual-1) == 0):
                        print('iters', iters)
                        # print('loss', loss)
            ...
            if model.imp_method == 'A-GEM':
                if COUNT_VIOLATONS:
                    ...
                    # sess.run(model.reset_violation_count_AECODER, feed_dict=feed_dict)
            ...
            if (task < (len(datasets) - 1)) or MEASURE_PERF_ON_EPS_MEMORY:
                model.task_updates(sess, task, train_x, np.arange(TOTAL_CLASSES)) # TODO: For MAS, should the gradients be for current task or all the previous tasks
            ...
            if args.train_single_epoch and not args.cross_validate_mode: 
                ...
                if MEASURE_PERF_ON_EPS_MEMORY:
                    ...
                    ftask = test_task_sequence(model, sess, eps_mem, task_labels, classes_per_task=classes_per_task)
                else:
                    # List to store accuracy for all the tasks for the current trained model
                    ftask = test_task_sequence(model, sess, datasets, task_labels)
...
def test_task_sequence(model, sess, test_data, test_tasks, classes_per_task=0):
    """
    ...
    if MEASURE_PERF_ON_EPS_MEMORY:
        for task in range(model.num_tasks):
            ...
            feed_dict = {model.x: test_data['images'][mem_offset:mem_offset+SAMPLES_PER_CLASS*TOTAL_CLASSES],
                    model.y_: test_data['labels'][mem_offset:mem_offset+SAMPLES_PER_CLASS*TOTAL_CLASSES], model.keep_prob: 1.0, model.train_phase: False, model.output_mask: logit_mask}
    ...
    for task in range(model.num_tasks):
        ...
        if model.imp_method == 'PNN':
            ...
            """logit_mask[:] = 0
            logit_mask[tt][labels] = 1.0"""
            feed_dict = {model.x: test_data[task]['test']['images'], model.y_: test_data[task]['test']['labels'], model.keep_prob: 1.0, model.output_mask: logit_mask, model.train_phase: False}
            """logit_mask_dict = {m_t: i_t for (m_t, i_t) in zip(model.output_mask, logit_mask)}
            feed_dict.update(logit_mask_dict)"""
            ...
            logit_mask[labels] = 1.0
            feed_dict = {model.x: task_test_images, 
                    model.y_: task_test_labels, model.keep_prob: 1.0, model.train_phase: False, model.output_mask: logit_mask, model.adv_noise: AdvLnoise, model.FM_h: perturbFM_h}
            acc = model.accuracy.eval(feed_dict = feed_dict)"""
...
def main():
    ...
    eps = 7
    sensitivity = 1
    experiment_id = "SPLIT_CIFAR_HERDING_2tasks_%s_%r_%s_%s_lr%s_bs%s_ns%s_eps%s_s%s_%s-"%(args.arch, args.train_single_epoch, args.imp_method, 
            str(args.synap_stgth).replace('.', '_'), str(args.learning_rate), 
            str(args.batch_size), str(noise_scale), str(eps), str(sensitivity), str(args.mem_size)) + datetime.datetime.now().strftime("%y-%m-%d-%H-%M")
    ...
    #datasetsTemp = construct_permute_mnist(num_tasks)
    datasets = construct_permute_cifar100("cifar-10-batches-bin/", num_tasks)
    ...
    if args.cross_validate_mode:
        ...
        # noise_scale = 2.5
        # eps = 4
        cross_validate_dump_file = args.log_dir + '/' + 'SPLIT_CIFAR_%s_%s'%(args.imp_method, args.optim) + '_bs' + str(args.batch_size) + '_ns' + str(noise_scale) + '_eps' + str(eps) + '.txt'
