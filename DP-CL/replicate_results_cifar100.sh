...
if [[ $DATASET = "MNIST" ]]; then
    ...
    for ((i=0;i<${#IMP_METHODS[@]};++i)); do
        echo "RUNNING fc_permute_mnist.py"
    ...
    for ((i=0;i<${#IMP_METHODS[@]};++i)); do
        ...
        lr=${LRS[i]}
        lam=${LAMDAS[i]}
        python ./fc_permute_cifar_100.py --train-single-epoch --arch $ARCH --num-runs $NUM_RUNS --batch-size $BATCH_SIZE --optim $OPTIM --learning-rate $lr --imp-method $imp_method --synap-stgth $lam --log-dir $RESULTS_DIR
