                ...
                if model.imp_method == 'VAN':
                    ...
                    if task == 0:
                        ...
                        if episodic_filled_counter <= args.eps_mem_batch:
                            ...
                            mem_sample_mask = np.random.choice(episodic_filled_counter, args.eps_mem_batch, replace=False) 
                       
                        # Store the reference gradient
                        
                        randomTaskForGrefComputation = random.randrange(task)
                        
                        sess.run(model.store_ref_grads, feed_dict={model.x: episodicMemoryPerTask[randomTaskForGrefComputation][0][:50], model.y_: episodicMemoryPerTask[randomTaskForGrefComputation][1][:50],
